...
export function drawDrawingScene(ctx:CanvasRenderingContext2D,p:DrawingProject,scene:DrawingScene,images:RasterCache,seconds=0,selectedId?:string,showTarget=false){
 ...
 for(const b of bubbles){ctx.save();ctx.font='500 24px "S-Core Dream",sans-serif';const lines=wrap(ctx,b.item.speech,270),height=lines.length*32+30,width=302,x=Math.max(8,Math.min(w-width-8,b.x-width/2)),y=Math.max(8,b.y-b.height-height-20);ctx.fillStyle='white';ctx.strokeStyle='#31445d';ctx.lineWidth=2;ctx.beginPath();ctx.roundRect(x,y,width,height,18);ctx.fill();ctx.stroke();ctx.fillStyle='#20364f';lines.forEach((line,i)=>ctx.fillText(line,x+16,y+34+i*32));ctx.restore();}
 if(scene.caption){ctx.save();ctx.font='500 24px "S-Core Dream",sans-serif';const lines=wrap(ctx,scene.caption,w-100);ctx.fillStyle='rgba(255,255,255,.92)';ctx.fillRect(20,20,w-40,lines.length*32+24);ctx.fillStyle='#20364f';lines.forEach((line,i)=>ctx.fillText(line,44,53+i*32));ctx.restore();}
}
