...
const summary=[],wait=ms=>new Promise(r=>setTimeout(r,ms)),key='moakit-play-project-v1';
...
try{
  ...
  for(const [name,engine]of[['chromium',chromium],['webkit',webkit]]){
    ...
    try{
      ...
      await page.getByRole('tab',{name:/소품/}).click();
      assert.equal(await page.locator('.prop-card svg').count(),12);
      await page.locator('.prop-card').filter({has:page.locator('strong',{hasText:'로봇'})}).click();
      ...
      const object=page.getByRole('button',{name:'로봇 이동',exact:true}),box=await object.boundingBox();assert(box);
      ...
      assert((await saved(page)).scenes[0].items.find(i=>i.kind==='prop'&&i.data.catalogId==='robot').x>56);
      ...
      let promise=page.waitForEvent('download');await page.getByRole('button',{name:'작품 파일 보관',exact:false}).click();const backup=await promise,backupPath=path.join(out,`${name}-work.moakit.json`);await backup.saveAs(backupPath);assert.equal(JSON.parse(await fs.readFile(backupPath,'utf8')).scenes.length,2);
      promise=page.waitForEvent('download',{timeout:60000});await page.getByRole('button',{name:'현재 장면 PNG',exact:false}).click();const image=await png(await promise,`${name}-scene.png`);assert.equal(image.width,2560);assert.equal(image.height,1440);
      promise=page.waitForEvent('download');await page.getByRole('button',{name:'전체 장면 PNG',exact:false}).click();assert((await png(await promise,`${name}-storyboard.png`)).width>image.width);
      ...
      await page.getByLabel('이름 / 모둠',{exact:true}).fill('샘플 · 별빛 모둠');await page.getByLabel('학급 / 수업',{exact:true}).fill('초등학교 2학년');await page.getByLabel('이 장면에서 할 행동',{exact:true}).first().fill('상자를 조심스럽게 열어 본다.');
      ...
      await page.screenshot({path:path.join(out,`${name}-outputs.png`),fullPage:true});await page.getByRole('button',{name:'출력창 닫기',exact:true}).click();
      page.on('dialog',d=>d.accept());await page.locator('input[type=file]').setInputFiles(backupPath);assert.equal((await saved(page)).scenes.length,2);
      const before=await saved(page);await page.locator('input[type=file]').setInputFiles({name:'broken.json',mimeType:'application/json',buffer:Buffer.from('{"version":1,"scenes":[{}]}')});await page.locator('.export-error').filter({visible:true}).waitFor();assert.deepEqual((await saved(page)).scenes,before.scenes);
      await page.getByRole('button',{name:'▶ 발표하기',exact:true}).click();await page.getByRole('dialog',{name:'연극 발표 화면',exact:true}).waitFor();await page.getByRole('button',{name:'편집으로 돌아가기 ×',exact:true}).click();await page.reload();assert.equal((await saved(page)).scenes.length,2);
      for(const viewport of[{width:1024,height:768},{width:390,height:844}]){await page.setViewportSize(viewport);await page.waitForTimeout(200);await page.screenshot({path:path.join(out,`${name}-${viewport.width}.png`),fullPage:true});const sizes=await page.evaluate(()=>[document.documentElement.scrollWidth,innerWidth]);assert(sizes[0]<=sizes[1]+1,`${name} horizontal overflow at ${viewport.width}: ${sizes}`);}
      assert.deepEqual(errors,[]);
      const badPage=await context.newPage();await badPage.addInitScript(k=>localStorage.setItem(k,'broken-work'),key);await badPage.goto('http://localhost:3100');await badPage.locator('.save-notice').waitFor();await wait(500);assert.equal(await badPage.evaluate(k=>localStorage.getItem(k),key),'broken-work');await badPage.close();
      ...
      await context.close();
    }catch(error){if(page){await page.screenshot({path:path.join(out,`${name}-failure.png`),fullPage:true}).catch(()=>{});await fs.writeFile(path.join(out,`${name}-failure.txt`),await page.locator('body').innerText().catch(()=>''));}summary.push({browser:name,result:'FAIL',error:String(error)});throw error;}
