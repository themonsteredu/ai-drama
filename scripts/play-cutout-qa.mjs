...
try{
  ...
  for(const [name,engine] of [['chromium',chromium],['webkit',webkit]]){
    ...
    try{
      ...
      await modal.getByLabel('흰 종이 지우기',{exact:true}).check();
      await modal.getByRole('button',{name:'지우기',exact:true}).click();await point(page,board,.5,.5);
      await modal.getByRole('button',{name:'무대에 놓기',exact:true}).click();await modal.waitFor({state:'hidden'});
      ...
      await modal.getByRole('button',{name:'다시 살리기',exact:true}).click();await point(page,board,.5,.5);
      ...
      await modal.getByRole('button',{name:'자르기',exact:true}).click();await drag(page,board,.20,.10,.80,.92);
      ...
      await modal.getByRole('button',{name:'수정 적용',exact:true}).click();await modal.waitFor({state:'hidden'});doc=await saved(page);asset=doc.assets[0];assert.equal(doc.assets.length,1,'re-edit updates same asset');assert.equal(doc.scenes[0].items.length,1,'re-edit does not add placements');assert.equal(asset.edit.original,original);assert(asset.edit.crop.width<300&&asset.edit.crop.width>150);const cx=Math.floor(180-asset.edit.crop.x),cy=Math.floor(150-asset.edit.crop.y);assert.equal((await pixels(page,asset.source,cx,cy))[3],255,'restore survives crop');
      await page.getByRole('button',{name:'초록친구 다듬기',exact:true}).click();await modal.waitFor();await page.waitForFunction(()=>document.querySelector('.cutout-dialog canvas')?.width===360);
      const unchanged=JSON.stringify(await saved(page));await modal.getByRole('button',{name:'전체 사진',exact:true}).click();await modal.getByRole('button',{name:'취소',exact:true}).click();await modal.waitFor({state:'hidden'});assert.equal(JSON.stringify(await saved(page)),unchanged,'cancel must not change project');
      ...
      for(const width of [1024,390]){await page.setViewportSize({width,height:950});await page.waitForTimeout(100);assert(await modal.evaluate(e=>e.scrollWidth<=e.clientWidth+2),`modal overflow ${name} ${width}`);await modal.screenshot({path:path.join(out,`${name}-cutout-${width}.png`)});}
      await page.setViewportSize({width:1440,height:1050});await modal.getByRole('button',{name:'처음으로',exact:true}).click();await modal.getByRole('button',{name:'배경 지우기',exact:true}).click();
      ...
      await modal.getByRole('button',{name:'수정 적용',exact:true}).click();await modal.waitFor({state:'hidden'});asset=(await saved(page)).assets[0];assert.equal((await pixels(page,asset.edit.mask,0,0))[0],0);assert.equal((await pixels(page,asset.edit.mask,180,150))[0],255,'GrabCut keeps foreground');
      reports.push({browser:name,result:'PASS',checks:['rectangle crop','erase alpha','restore after reopening','immutable original','original comparison','undo/redo','cancel unchanged','backup and reload','edit URL validation','mobile/tablet modal layout','real OpenCV GrabCut']});
